...
*   **Headers Required:**
    *   `DPoP`: `<jwt_proof>` — The cryptographically signed proof generated by the browser.
    <!-- *   `X-BFF-Secret`: `<your_secret>` — Injected by the Next.js Server Action to prove the request is proxied securely.  -->
...
*   **Processing:**
    ...
    <!-- 6. **(Optional but Recommended)** Check for the `X-BFF-Secret` header to ensure the request came through the Next.js frontend proxy and not from a direct attacker script. -->
